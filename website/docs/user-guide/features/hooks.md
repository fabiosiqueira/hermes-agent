...
| `message:sent` | A reply was delivered to the chat platform. Fires once per delivered reply, on both the plain-send and streaming paths — `agent:end` says a turn finished, this says something actually reached the chat. | `platform`, `chat_id`, `thread_id`, `chat_type`, `user_id`, `session_key`, `origin` (`"command"` = the operator driving the agent from chat, by slash command or free-form message; `"internal"` = a gateway self-injected notification), `slash_command` (name without the leading `/`, empty for free-form), `messages` (platform messages this delivery occupied — a long reply is split across several; `0` when the send failed), `chars`, `success` |
